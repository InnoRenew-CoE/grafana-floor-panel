...
export const sampleData: QueryData = JSON.parse<QueryData>(`{
  "state": "Done",
  "series": [
    {
      "refId": "A",
...
export const sampleSensorData: SensorData[] = sampleData.series.reduce((data, series) => {
    const fields = series.fields;
    const fieldOrder = fields.find(x => x.name === "_field").values;
    return mapSensorData(fieldOrder, [...data, ...fields.filter(x => x.name !== "_field")]);
}, []);

function mapSensorData(fieldOrder: string[], fields: Field[]) {
    const time = fields[0].labels._time
    return fields.map(sensor => {
